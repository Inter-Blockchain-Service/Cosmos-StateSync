...
set -e
REPO="https://github.com/desmos-labs/desmos"
...
sleep 1
  ...
  sed -E -i -s 's/minimum-gas-prices = \".*\"/minimum-gas-prices = \"0.0025udsm\"/' $DAEMON_HOME/config/app.toml
  ...
  echo "##################################################################"
  echo             $DAEMON_HOME is installed and synced
  echo "##################################################################"
